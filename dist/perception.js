import {RETINA_WIDTH as W,RETINA_HEIGHT as H,EYE_PIXELS,EMBODIED_INPUTS} from './sensory-inputs.js?v=9.2';
import {PLANET_RADIUS as R,PLANET_MU as MU} from './orbital.js?v=9.2';
...
// Rays from two eye positions intersect the three physical monitor planes.
// This small, deterministic optical renderer runs identically in training workers.
export function renderRetinas(s,screens){
 const images=[],pitch=-1.5,up=[0,Math.cos(pitch),Math.sin(pitch)],normal=[0,-Math.sin(pitch),Math.cos(pitch)];
 for(const side of [-1,1]){const im=image(W,H),eye=[side*.16,2.68,.32],yaw=(s.gaze??0)*.52;
  for(let y=0;y<H;y++)for(let x=0;x<W;x++){
   const local=norm([(x+.5-W/2)/(H*.70),(H/2-y-.5)/(H*.70),1]);
   const down=-.74,c=Math.cos(down),sn=Math.sin(down),dy=c*local[1]+sn*local[2],dz=-sn*local[1]+c*local[2];
   const ray=[Math.cos(yaw)*local[0]+Math.sin(yaw)*dz,dy,-Math.sin(yaw)*local[0]+Math.cos(yaw)*dz];let color=[4,8,12];
   for(let k=0;k<3;k++){const center=add([(k-1)*1.35,1.22,1.77],mul(normal,.066)),t=dot(center.map((v,i)=>v-eye[i]),normal)/dot(ray,normal);if(t<=0)continue;const p=add(eye,mul(ray,t)).map((v,i)=>v-center[i]),u=p[0],v=dot(p,up);if(Math.abs(u)<=.54&&Math.abs(v)<=.33){const ix=clamp(Math.floor((.5-u/1.08)*64),0,63),iy=clamp(Math.floor((.5+v/.66)*48),0,47),j=(iy*64+ix)*4;color=Array.from(screens[k].data.slice(j,j+3));}}
   pixel(im,x,y,s.eyesCovered?[0,0,0]:color);
  }images.push(im);
 }return images;
}
...
export function sampleEmbodied(s){
 if(s.perception?.step===s.step&&s.perception.covered===!!s.eyesCovered)return s.perception;
 ...
 s.perception={step:s.step,time:s.t,covered:!!s.eyesCovered,observations:Array.from(observations),raw:[...means.map(v=>v*100),...body.map((v,i)=>i===0||i===8?(v+1)*50:i<10?v*100:v)],display,eyes,screens,retinalChange:change,odor:{acetate:odor.acetate,geosmin:odor.geosmin,events:odor.events.map(e=>({...e}))}};return s.perception;
}
