import {sampleEmbodied,paintImage} from './perception.js?v=9.6';
import {FLIGHT_PANEL,INSTRUMENT_FIELDS} from './flight-instruments.js?v=9.6';
import {EMBODIED_CONTROLLER,EMBODIED_SCHEMA,EMBODIED_INPUTS} from './sensory-inputs.js?v=9.6';
import {DT,physicsStep,decisionSteps,SCENARIOS,clamp,createFlight,fullSensors,advance,actionTargets} from './engine3d.js?v=9.6';
import {ORBIT_PHASES,orbitalElements} from './orbital.js?v=9.6';
import {missionFamily,missionOptions,familyProfiles} from './missions.js?v=9.6';
import {drawChart} from './scene.js?v=9.6';
import {FlightScene3D as FlightScene,Cockpit3D as CockpitView} from './scene3d.js?v=9.6';
import {ConnectomeView} from './connectome-view.js?v=9.6';
import {captureDecision,DecisionView} from './decision.js?v=9.6';
import {WiringView} from './wiring-view.js?v=9.6';
import {CONTROL_LIMBS} from './kinematics.js?v=9.6';
import {FlightPresentation} from './presentation.js?v=9.6';
import {CONTROLS} from './decision.js?v=9.6';
import {validCheckpoint,freshCheckpoint,freshEmbodied,isEmbodied,CONTROLLER_ID} from './full-controller.js?v=9.6';

import {SENSOR_SCHEMA} from './signals.js?v=9.6';
...
function telemetry(){
  ...
  bipolar('gimbal-output',s.gimbal/.22);bipolar('rcs-output',s.rcs);$('gimbal-value').textContent=`${(s.gimbal*180/Math.PI).toFixed(1)}°`;$('rcs-value').textContent=`${Math.round(s.rcs*100)}%`;
...
async function boot(){
  try {
    ...
    const load=async path=>{const r=await fetch(path+(path.includes('?')?'':'?v=9.6'),{cache:'no-cache'});if(!r.ok)throw new Error(`Could not load ${path}`);return r.json();};
    ...
    [graduate,specialist,expert,orbitalPilot]=await Promise.all(['full-pilot','full-specialist','full-expert','full-orbital'].map(name=>load(`assets/${name}.json?v=9.6`)));
    ...
    worker=new Worker('trainer-worker.js?v=9.6',{type:'module'});
...
function bootConnectome(){
  ...
  networkWorker=new Worker('connectome-worker.js?v=9.6',{type:'module'});
