...
let network,weights,flightId=-1,lastDecision;
onmessage=async({data})=>{
 try{
  if(data.type==='start'){
   ...
   const values=network.activity.slice();postMessage({type:'decision',commands,step:data.step,flightId,values,stats:network.stats(),milliseconds:performance.now()-started},[values.buffer]);
