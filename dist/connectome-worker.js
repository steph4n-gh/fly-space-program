import {loadNetwork} from './full-network.js?v=9.2';
...
onmessage=async({data})=>{
 try{
  ...
  }else if(data.type==='reset'&&network){network.setInputMode(data.inputMode??'telemetry');network.setActivationGain(data.activationGain??1);network.reset();weights=Float64Array.from(data.weights);flightId=data.flightId;lastDecision=null;}
