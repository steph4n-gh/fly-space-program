...
let network,drive={observations:Array(8).fill(0),coreActivity:Array(96).fill(0),flightId:0,paused:false},running=false,lastFlight=-1,tickIndex=0;
async function boot(){
...
function tick(){
  if(!running)return;
  if(!drive.paused){
    if(lastFlight!==drive.flightId){network.reset();lastFlight=drive.flightId;tickIndex=0;}
    const started=performance.now(),stats=network.step(drive.observations,drive.coreActivity),elapsed=performance.now()-started;
    const values=network.activity.slice(),feedback=network.feedback.slice();postMessage({type:'activity',values,feedback,stats,milliseconds:elapsed,flightId:drive.flightId,tick:++tickIndex},[values.buffer,feedback.buffer]);
