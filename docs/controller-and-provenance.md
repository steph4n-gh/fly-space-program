# Controller architecture and evidence history

[Project overview](../README.md) · [Documentation index](README.md) · [Reproduction guide](reproducibility.md)

This detailed note preserves the controller design and checkpoint history. For the latest experiment status, follow the [research guide](README.md#research-record).

An experimental fruit-fly connectome controls a 3D booster. The default **Perceiving fly** receives light at two modeled eyes, body feedback and receptor-specific odor stimuli. Its cockpit presents a camera and twelve measured flight indicators. These measurements reach the graph through eye pixels; no numerical navigation array or target trajectory enters the controller. See [Learning the mission suite](suite-training.md) for the training procedure, frozen-checkpoint tests and limitations.
...
in its [historical report](assets/ten-mission-g6-report.json).
...
[first landing lesson](landing-training.md) achieved 6/24 on its separate
stationary-deck test; that was a different checkpoint and test set.

The physical-interface research target, 27-mission baseline, expanded 689-odor atlas, complete-graph spiking experiments and odor-controller trials are documented in [A physically testable fly interface](physical-fly-interface.md). The additional [eight-compound panel](additional-odor-tests.md) and [calibration follow-up](odor-calibration-followup.md) did not establish chemical steering. [Harness mockups](harness-concept.md) explore a 125-cell cube and individual fly chambers. The current software is not a validated predictor of a living fly's behavior.
...
The [diagnostic record](visual-transmission-results.json) preserves all
phase counts and voltage traces.

An [eight-trial follow-up](visual-transmission-background-results.json)
