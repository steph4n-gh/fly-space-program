...
An experimental fruit-fly connectome controls a 3D booster. The default **Perceiving fly** receives light at two modeled eyes, body feedback and receptor-specific odor stimuli. Its first landing curriculum uses a camera and visible light indicators for measured clearance and vertical speed. These measurements reach the graph through eye pixels; no numerical navigation array or target trajectory enters the controller. See [Learning the first landing](docs/landing-training.md) for the training procedure, frozen-checkpoint tests and limitations.

The shipped landing checkpoint achieved **6/24 safe landings on unseen starts**
(5/12 nominal, 1/12 varied), versus **0/24** for the earlier visual checkpoint.
Covering its eyes or disabling the landing indicators reduced it to **0/24**.
This establishes a limited first landing skill on the stationary deck. It does
not establish reliable flight, other mission skills or transfer to a living fly.

The physical-interface research target, 27-mission baseline, expanded 689-odor atlas, complete-graph spiking experiments and 54 odor-controller training/test trials are documented in [A physically testable fly interface](docs/physical-fly-interface.md). These experiments have not established reliable steering or improved landings. The current software is not a validated predictor of a living fly's behavior.
...
A deterministic optical renderer works identically in browser and offline workers. A fixed vehicle-mounted downward camera sees the geometric deck, ocean/spherical planet and sky, with night/fog affecting received light. It never aims itself at a hidden target. The center cockpit monitor displays that image and two optional light indicators: clearance from 0–150 m above touchdown and vertical speed from −25 to +25 m/s. Fuel and engine lights appear on the side monitors. Each eye pixel averages a 4×4 grid of rays intersecting those physical monitor planes. Head yaw changes the received image. The cockpit’s pixel previews are the exact sampled images, before presentation interpolation. This is a simplified optical scene, not a biological compound-eye optical model or the high-detail spectator renderer.
...
The embodied controller starts independently of the old instrument-trained checkpoints. The landing curriculum calibrates full-network motor activity against presented indicator brightness and body feedback, then learns throttle from complete-flight reward. A learned sensory basis is folded into the ordinary 21,300-weight readout. Pitch, roll, yaw, fins and gaze remain neutral for the stationary-deck lesson. Anatomical connections stay fixed. The other 26 missions remain untrained.
